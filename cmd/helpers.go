...
func removeEligibleTorrents(log *logrus.Entry, c client.Interface, torrents map[string]config.Torrent,
	...
	// Create a slice from the map: (Total torrent)
	...
	sort.Slice(removeTorrents, func(i, j int) bool {
		return removeTorrents[i].Torrent.AddedSeconds < removeTorrents[j].Torrent.AddedSeconds
	})
	...
	if removeCount < 0 {
		for _, pair := range removeTorrents {
			h := pair.Hash
			t := pair.Torrent
